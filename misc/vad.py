...
class VoiceActivityDetector():
    ...
    def __init__(self, rate, data, speech_energy_threshold):
        self._read_wav(rate, data)._convert_to_mono()
    ...
    def convert_windows_to_readible_labels(self, detected_windows):
        ...
        Output updated to return array of start and end samples of max length speech
        """
        is_speech = 0
        starts = []
        ends = []
        for window in detected_windows:
            ...
            if window[1] == 0.0 and is_speech == 1:
                ends.append(window[0])
                is_speech = 0

        # check to see set end time to end of file if no end time registered
    ...
    def detect_speech(self):
        ...
        while (sample_start < (len(data) - sample_window)):
            ...
            if np.isclose(sum_full_energy, 0):
                speech_ratio = 0
            else:
