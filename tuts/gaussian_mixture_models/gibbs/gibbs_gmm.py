...
"""
# !/usr/bin/env python
# https://github.com/EricBataller/Gibbs_Sampling_MC/blob/master/Gibbs_sampler_GMM.py
...
class MixGaussGibbsSampler():
    def __init__(self, X, Z, m0=None, V0=None, nu0=None, S0=None, alpha=None, burn_in=50):
        ...
        diagsig = np.zeros(
            (self.num_mixtures, self.dim, self.dim))  # Diagonal scatter matix .We will need it to initialize S0
        ...
        if not V0:  # if I have no prior info about the variance of mu
            ...
            for k in range(0, self.num_mixtures):
                listI.append(I)
            self.V0 = np.array(listI) * 100000  # uninformative mu prior --> V0 = inf*I
            self.invV0 = np.array(listI) * (1 / 100000)
        else:
            self.V0 = V0
        ...
        if not alpha:  # if I have no info about the Dirichlet prior
            self.alpha = (1e-3 * np.ones(
                self.num_mixtures) / self.num_mixtures)  # we set a concave distribution in the simplex with max probability around the center --> more or less same amount of points for each gaussian
        else:
    ...
    def sample_z(self):
        ...
        for i in range(0, self.num_points):
            ...
            for k in range(0, self.num_mixtures):
                ...
                self.nk[k] = max(self.nk[k], epsilon)
        pass
    ...
    def plot_points(self, title):
        ...
        for j, data in enumerate(datasets):
            ax.scatter(data[:, 0], data[:, 1], color=colours[j], cmap=plt.cm.Set1, edgecolor='k')
...
if __name__ == "__main__":
    ...
    d = {'sepal length': Y[:, 0], 'sepal width': Y[:, 1], 'petal length': Y[:, 2], 'petal width': Y[:, 3],
         'assig': gs.Z}
